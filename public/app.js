...
/* ---- Cash / GPay payment modal ----
   Simplified on purpose: staff just record how the customer paid and the
   bill prints immediately — no "Generate QR" step. The counter already has
   a static UPI QR poster for customers who want to scan directly; this
   screen is only for the till record, not for generating a fresh QR. */
function openPaymentModal(ctx) {
  ...
  <div class="modal-overlay" id="pay-modal-overlay">
    <div class="modal-box" style="text-align:center;">
      ...
      <div style="display:flex;gap:10px;margin:14px 0;">
        <button class="checkout-btn pay-cash" id="pay-cash-btn" style="flex:1;">💵 Cash</button>
        <button class="checkout-btn pay-gpay" id="pay-gpay-btn" style="flex:1;">📲 GPay</button>
      </div>
  ...
  document.getElementById('pay-gpay-btn').addEventListener('click', () => { closeModal(); submitOrder(ctx, 'gpay'); });
}
...
async function renderReportsTab() {
  ...
  // Range-filtered slice, used by the payment-mix card, top-sellers list,
  // the table below, and the CSV export — so everything on screen matches
  // whichever From/To dates staff picked.
  const rangeOrders = orders.filter(o => {
    const d = todayStr(o.time);
    return d >= REPORT_FROM && d <= REPORT_TO;
  });
  const rangeTotal = rangeOrders.reduce((s, o) => s + o.total, 0);
  const cashTotal = rangeOrders.filter(o => o.paymentMethod !== 'gpay').reduce((s, o) => s + o.total, 0);
  const gpayTotal = rangeOrders.filter(o => o.paymentMethod === 'gpay').reduce((s, o) => s + o.total, 0);
  const cashCount = rangeOrders.filter(o => o.paymentMethod !== 'gpay').length;
  const gpayCount = rangeOrders.filter(o => o.paymentMethod === 'gpay').length;

  // Top-selling items across the selected range (qty and revenue), the
  // "special feature" — tells staff/owner what's actually moving.
  const itemStats = new Map();
  rangeOrders.forEach(o => (o.items || []).forEach(it => {
    const key = it.name;
    const cur = itemStats.get(key) || { name: it.name, qty: 0, revenue: 0 };
    cur.qty += Number(it.qty) || 0;
    cur.revenue += (Number(it.qty) || 0) * (Number(it.price) || 0);
    itemStats.set(key, cur);
  }));
  const topItems = [...itemStats.values()].sort((a, b) => b.qty - a.qty).slice(0, 5);
  const maxQty = topItems.length ? topItems[0].qty : 1;

  const recent = [...rangeOrders].reverse().slice(0, 60);
  ...
  <div class="reports-head" style="margin-top:6px;">
    <h3 style="margin:0;">Custom Range</h3>
  </div>
  <div class="filters">
    <div class="field"><label>From</label><input type="date" id="rep-from" value="${REPORT_FROM}"></div>
    <div class="field"><label>To</label><input type="date" id="rep-to" value="${REPORT_TO}"></div>
    <button class="tiny-btn" id="rep-apply-btn">Apply</button>
    <button class="tiny-btn" id="rep-export-btn">⬇️ Export CSV</button>
  </div>

  <div class="grid-cards">
    <div class="stat-card"><div class="lbl">Bills in Range</div><div class="val">${rangeOrders.length}</div></div>
    <div class="stat-card"><div class="lbl">Sales in Range</div><div class="val">${rupee(rangeTotal)}</div></div>
    <div class="stat-card"><div class="lbl">💵 Cash (${cashCount})</div><div class="val">${rupee(cashTotal)}</div></div>
    <div class="stat-card gold"><div class="lbl">📲 GPay (${gpayCount})</div><div class="val">${rupee(gpayTotal)}</div></div>
  </div>

  <div class="table-wrap" style="margin-bottom:22px;">
    <h3 style="margin:6px 0 10px;font-size:18px;">🏆 Top-Selling Items</h3>
    ${topItems.length ? topItems.map(it => `
      <div class="top-item-row">
        <div class="top-item-name">${escapeHtml(it.name)}</div>
        <div class="top-item-bar-wrap"><div class="top-item-bar" style="width:${Math.max(6, Math.round((it.qty / maxQty) * 100))}%"></div></div>
        <div class="top-item-qty">${it.qty} sold</div>
        <div class="top-item-rev">${rupee(it.revenue)}</div>
      </div>
    `).join('') : `<p style="color:var(--muted);font-size:13px;">No items sold in this range yet.</p>`}
  </div>

  <div class="table-wrap">
    <table>
      <thead><tr><th>Bill#</th><th>Counter</th><th>Time</th><th>Items</th><th>Note</th><th>Customer</th><th>Customer #</th><th>Served By</th><th>Payment</th><th>Total</th><th></th></tr></thead>
      <tbody>
        ${recent.map(o => `
          <tr>
            ...
            <td>${o.servedBy}</td>
            <td>${o.paymentMethod === 'gpay' ? '📲 GPay' : '💵 Cash'}</td>
            <td>${rupee(o.total)}</td>
            <td><button class="tiny-btn" data-reprint="${o.billNo}">Reprint</button></td>
          </tr>
        `).join('') || `<tr><td colspan="11" style="color:var(--muted);text-align:center;">No bills in this range.</td></tr>`}
      </tbody>
    </table>
  </div>`;
  ...
  document.getElementById('rep-apply-btn').addEventListener('click', () => {
    const f = document.getElementById('rep-from').value;
    const t = document.getElementById('rep-to').value;
    if (f) REPORT_FROM = f;
    if (t) REPORT_TO = t;
    if (REPORT_FROM > REPORT_TO) { const tmp = REPORT_FROM; REPORT_FROM = REPORT_TO; REPORT_TO = tmp; }
    renderReportsTab();
  });
  document.getElementById('rep-export-btn').addEventListener('click', () => exportOrdersCsv(rangeOrders));
}

/* Downloads the currently filtered range as a CSV — handy for owners who
   want to check the day's numbers in Excel/Google Sheets or hand it to
   an accountant. */
function exportOrdersCsv(orders) {
  const headers = ['Bill No', 'Counter', 'Date', 'Time', 'Items', 'Note', 'Customer', 'Customer Mobile', 'Served By', 'Payment', 'Total'];
  const csvEscape = v => `"${String(v == null ? '' : v).replace(/"/g, '""')}"`;
  const rows = orders.map(o => [
    formatBillCode(o),
    counterLabel(o.menuKey),
    billDateStr(o.time),
    billTimeStr(o.time),
    ...
    o.customerMobile || '',
    o.servedBy || '',
    o.paymentMethod === 'gpay' ? 'GPay' : 'Cash',
    o.total
  ].map(csvEscape).join(','));
  ...
  const a = document.createElement('a');
  a.href = url;
  a.download = `sales_${REPORT_FROM}_to_${REPORT_TO}.csv`;
  document.body.appendChild(a);
  a.click();
  a.remove();
